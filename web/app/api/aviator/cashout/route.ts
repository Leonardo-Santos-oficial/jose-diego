...
export async function POST(request: Request) {
  ...
  const kind = payload.kind === 'auto' ? 'auto' : 'manual';
  ...
  try {
    ...
    const status = result.status === 'credited' ? 200 : 400;
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Falha ao processar cashout.' },
      { status: 500 }
    );
  }
}
