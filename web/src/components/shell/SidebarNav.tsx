...
export function SidebarNav({ isAuthenticated }: SidebarNavProps) {
  return (
    <aside
      className="flex min-w-0 flex-col gap-3 border-r border-slate-800/60 bg-slate-950/80 p-6 shadow-inner lg:min-h-screen"
    ...
    >
      {links.map((link) => (
        ...
        >
          ...
          <small className="block text-sm text-slate-400">{link.description}</small>
        </a>
      ))}
    </aside>
  );
