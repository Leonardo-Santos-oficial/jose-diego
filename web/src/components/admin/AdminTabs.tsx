...
export function AdminTabs({ tabs, activeTab, onTabChange }: AdminTabsProps) {
  return (
    <nav className="-mx-4 px-4 flex gap-2 overflow-x-auto border-b border-slate-800/60 pb-px scrollbar-thin scrollbar-thumb-slate-700 md:mx-0 md:px-0">
      {tabs.map((tab) => (
        <button
          key={tab.id}
          onClick={() => onTabChange(tab.id)}
          className={cn(
            'flex items-center gap-2 whitespace-nowrap rounded-t-lg px-4 py-3 min-h-[52px] text-sm font-medium transition-colors',
            activeTab === tab.id
